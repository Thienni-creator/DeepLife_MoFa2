...
class FA(PyroModule):
    ...
    def model(self):
        ...
        with self.latent_factor_plate:
            ...
            with self.feature_plate1:
                ...
                W1_hat = pyro.sample("W1", pyro.distributions.Normal(0., 1./alpha1))*pyro.sample("s1", pyro.distributions.Bernoulli(theta1)).to(device)
            ...
            with self.feature_plate2:
                ...
                W2_hat = pyro.sample("W2",pyro.distributions.Normal(0., 1./alpha2))*pyro.sample("s2",pyro.distributions.Bernoulli(theta2)).to(device)
        ...
        # estimate for Y
        with pyro.plate("sub_sample", self.num_samples, subsample_size = subsample_size) as ind:
            Y1_hat = torch.matmul(Z[ind,:], W1_hat.t())
            Y2_hat = torch.matmul(Z[ind,:], W2_hat.t())
        ...
        with pyro.plate("feature1_", self.Y1.shape[1]):
            ...
            scale1 = pyro.sample("scale1", pyro.distributions.LogNormal(0., 1./(precision1))).to(device)
            with pyro.plate("sample_", subsample_size):
            # masking the NA values such that they are not considered in the distributions
                ...
                #         self.Y1 = torch.nan_to_num(self.Y1, nan=0)
                        ...
                        pyro.sample("obs1", pyro.distributions.Normal(Y1_hat, scale1), obs=self.Y1[ind]).to(device)
        ...
        with pyro.plate("feature2_", self.Y2.shape[1]): 
            ...
            scale2 = pyro.sample("scale2", pyro.distributions.LogNormal(0., 1./(precision2))).to(device)
            with pyro.plate("sample2_", subsample_size):
            # masking the NA values such that they are not considered in the distributions
                ...
                #         self.Y2 = torch.nan_to_num(self.Y2, nan=0) 
                        ...
                        #precision2 = 0.6
                        # compare sampled estimation to the true observation Y
                        pyro.sample("obs2", pyro.distributions.Normal(Y2_hat, scale2), obs=self.Y2[ind]).to(device)
    ...
    def train(self):
        ...
        #torch.save({"model": self.state_dict(), "guide" : guide}, "/mnt/storage/thien/FA_model_10_factors.pt")
        #pyro.get_param_store().save("/mnt/storage/thien/FA_model_params_10_factors.pt")
...
if __name__ == "__main__":
    ...
    pd.DataFrame(losses).to_csv("/mnt/storage/thien/actual_MoFa/FA_losses_10factors.csv")
