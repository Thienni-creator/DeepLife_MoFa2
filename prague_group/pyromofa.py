...
class MOFA(PyroModule):
    ...
    def train(self, lr=0.02, num_iterations = 4000):
        ...
        for j in range(num_iterations):
            ...
            if j % 200 == 0:
                print("[%02d:%02.1d iteration %05d] loss: %.4f" % (math.floor((time.time() - t0)/60), round(time.time() - t0, ndigits=1) % 60, j + 1, loss / self.num_samples))
    ...
    def train_epochs(self, lr=0.002, num_epochs = 20):
        """
        test3
        """
        # set training parameters
        ...
        for j in range(num_epochs):
            ...
            for batch_i in range(math.ceil(len(indices) / self.batch_size)):
                self.subsample_ix = indices[batch_i * self.batch_size: (batch_i + 1) * self.batch_size]
                # calculate the loss and take a gradient step
                # (loss should be already scaled down by the subsample_size)
                loss = svi.step()

                train_loss.append(loss/self.num_samples)
            print("[%02d:%02.1d iteration %02d:%05d] loss: %.4f" % (round((time.time() - t0)/60), round(time.time() - t0, ndigits=1) % 60, j+1, ((j+1)*math.ceil(len(indices) / self.batch_size)), loss / self.num_samples))
        # Obtain maximum a posteriori estimates for W and Z
        # map_estimates = guide(self.Y)  # not sure why needed Y?
        # "Note that Pyro enforces that model() and guide() have the same call signature, i.e. both callables should take the same arguments."
        with torch.no_grad():
